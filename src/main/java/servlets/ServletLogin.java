...
public class ServletLogin extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if( ( action != null || !action.equalsIgnoreCase("null") ) && !action.isEmpty() 
		...
		try {
			...
			if (login != null && !login.isEmpty() && password != null && !password.isEmpty()) {
				...
				if (daoLoginRepository.userAuthentication(modelLogin)) {
					...
					request.getSession().setAttribute("userImage", daoUser.consultUserByLogin( modelLogin.getLogin() ).getUserPhoto() );
					request.getSession().setAttribute("perfil", isUserAdmin.getPerfil());
					/*
					 * HERE IS POSSIBLE MAKE A VALIDATION TO SHOW FIELDS OF CUSTOMER BASE ONLY TO ADMINS PERFILS TOO
					 * request.getSession().setAttribute("isAdmin", isUserAdmin.getUserAdmin());
					*/
